...
async function getClaimParams(options) {
  const cc = await (async () => {
    if (options.type === "erc1155") {
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-BTHXJN5P.js");
    ...
    if (options.type === "erc721") {
      if (options.singlePhaseDrop) {
        const { claimCondition } = await import("./claimCondition-XCQR7CWV.js");
        return await claimCondition({
          contract: options.contract
        });
      }
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-SNTWGCCR.js");
      return await getActiveClaimCondition2({
        contract: options.contract
      });
    }
    ...
    const { getActiveClaimCondition } = await import("./getActiveClaimCondition-V74TATWR.js");
  ...
  const allowlistProof = await (async () => {
    ...
    const { fetchProofsForClaimer } = await import("./fetch-proofs-for-claimers-54SXGSFQ.js");
...
//# sourceMappingURL=chunk-WTVUONKS.js.map
